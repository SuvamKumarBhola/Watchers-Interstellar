...
const Dashboard = () => {
    ...
    const fetchFeed = async () => {
        try {
            ...
            // Augment data with fake risk scores for demo visualization since API doesn't provide 0-100 directly
            const augmentedData = (data.data || []).map(a => ({
                ...a,
                riskScore: a.is_potentially_hazardous ? Math.floor(Math.random() * (100 - 60) + 60) : Math.floor(Math.random() * 40)
            }));
            setAsteroids(augmentedData);
        } catch (err) {
